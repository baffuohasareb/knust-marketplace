...
export default function AddProductPage() {
  ...
  const updateVariationValue = (variationId: string, valueIndex: number, value: string) => {
    setVariations(prev => prev.map(v => 
      v.id === variationId 
        ? { 
            ...v, 
            values: v.values.map((val, i) => i === valueIndex ? value : val)
          }
        : v
    ));
  };
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden">
          <div className="px-8 py-6 border-b border-gray-200">
            ...
            <p className="text-gray-600">Add a {isService ? 'service' : 'product'} to {business.name}</p>
          </div>
                  ...
                  <div className="space-y-6">
                    <div>
                      ...
                      <input
                        ...
                        placeholder={isService ? "e.g., Mathematics Tutoring" : "e.g., Wireless Headphones"}
                    ...
                    <div className="space-y-4">
                      <div className="flex items-center space-x-3">
                        ...
                        <label htmlFor="isActive" className="text-sm font-medium text-gray-700">
                          Make this {isService ? 'service' : 'product'} active immediately
                        </label>
                      </div>
                ...
                <div>
                  ...
                  <div className="border-2 border-dashed border-gray-300 rounded-lg p-8 text-center hover:border-gray-400 transition-colors">
                    ...
                    <p className="text-sm text-gray-600 mb-2">
                      Upload {isService ? 'service' : 'product'} images (up to 10 images)
                    </p>
                    <input
                      type="file"
                      multiple
                    ...
                    <p className="text-xs text-gray-500 mt-2">
                      PNG, JPG up to 5MB each. First image will be the main image.
                    </p>
                  </div>
              ...
              <div className="space-y-6">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-sm text-gray-600 mt-1">
                      Add variations like size, color, material, etc. Each variation can have multiple options.
                    </p>
                  </div>
                  <button
                    type="button"
                ...
                {variations.length === 0 ? (
                  <div className="text-center py-12 bg-gray-50 rounded-lg">
                    ...
                    <h4 className="text-lg font-medium text-gray-900 mb-2">No variations added</h4>
                    <p className="text-gray-600 mb-4">
                      Add variations like size, color, or material to give customers more options
                    </p>
                    <button
                      type="button"
                      onClick={addVariation}
                  ...
                  <div className="space-y-6">
                    {variations.map((variation, variationIndex) => (
                      <div key={variation.id} className="bg-gray-50 rounded-lg p-6">
                              ...
                              {variation.values.map((value, valueIndex) => (
                                <div key={valueIndex} className="flex items-center space-x-2">
                                  <input
                                    type="text"
                                    value={value}
                                    onChange={(e) => updateVariationValue(variation.id, valueIndex, e.target.value)}
                                    placeholder={`${variation.name || 'Option'} ${valueIndex + 1}`}
                                    className="flex-1 px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent"
                                  />
                                  {variation.values.length > 1 && (
                                    <button
                                      type="button"
                                      onClick={() => removeVariationValue(variation.id, valueIndex)}
              ...
              <div className="space-y-6">
                <div>
                  ...
                  <p className="text-sm text-gray-600 mb-6">
                    Manage stock levels and inventory tracking for your {isService ? 'service' : 'product'}.
                  </p>
                </div>
                ...
                <div className="space-y-4">
                  <div className="flex items-center space-x-3">
                    ...
                    <label htmlFor="trackQuantity" className="text-sm font-medium text-gray-700">
                      Track quantity for this {isService ? 'service' : 'product'}
                    </label>
                  </div>
                  ...
                  <div className="flex items-center space-x-3">
                    ...
                    <label htmlFor="allowBackorders" className="text-sm font-medium text-gray-700">
                      Allow {isService ? 'bookings' : 'orders'} when out of stock
                    </label>
                  </div>
                </div>
              </div>
            ...
            {activeTab === 'shipping' && (
              <div className="space-y-6">
                <div>
                  ...
                  <p className="text-sm text-gray-600 mb-6">
                    Configure how your {isService ? 'service will be delivered' : 'product will be shipped'}.
                  </p>
                </div>

                <div className="bg-blue-50 rounded-lg p-6">
                  <h4 className="font-medium text-blue-900 mb-2">
                    {isService ? 'Service Delivery Options' : 'Shipping Information'}
                  </h4>
                    ...
                    {business.delivery?.available && (
                      <>
                        <div className="flex justify-between">
                          ...
                          <span className="font-medium">₵{business.delivery.fee || 0}</span>
                        </div>
                        <div className="flex justify-between">
                          <span>Coverage:</span>
                          <span className="font-medium">{business.delivery.coverage || 'Campus'}</span>
                        </div>
                      </>
                    )}
                  </div>
                ...
                {isService && (
                  <div className="bg-gray-50 rounded-lg p-6">
                    <h4 className="font-medium text-gray-900 mb-4">Service Delivery Method</h4>
                    <div className="space-y-3">
                      <label className="flex items-center space-x-3">
                        ...
                        />
                        <div>
                          <span className="font-medium text-gray-900">In-Person Service</span>
                          <p className="text-sm text-gray-600">Service provided at customer's location or your location</p>
                        </div>
                      </label>
                      ...
                      <label className="flex items-center space-x-3">
                        ...
                        />
                        <div>
                          <span className="font-medium text-gray-900">Online Service</span>
                          <p className="text-sm text-gray-600">Service provided remotely via video call or online platform</p>
                        </div>
                      </label>
                      ...
                      <label className="flex items-center space-x-3">
                        ...
                        <div>
                          ...
                          <p className="text-sm text-gray-600">Combination of in-person and online service</p>
                        </div>
                      </label>
                    </div>
                  </div>
              ...
              <div className="flex space-x-4">
                {activeTab !== 'basic' && (
                  <button
                    type="button"
                    onClick={() => {
                      const tabs = ['basic', 'variations', 'inventory', 'shipping'];
                ...
                {activeTab !== 'shipping' ? (
                  ...
                  >
                    {loading ? `Adding ${isService ? 'Service' : 'Product'}...` : `Add ${isService ? 'Service' : 'Product'}`}
                  </button>
                )}
              </div>
            </div>
